...
const members = [
  {
    ...
    filename: 'dr_v_narasimha_nayak.html',
    affiliation: 'Dr. B. R. Ambedkar National Institute of Technology, Jalandhar'
  },
  {
    name: 'Prof. Nuno Domingues',
    email: 'nndomingues@gmail.com',
    secondary_email: 'nuno_domingues@isel.pt',
    filename: 'prof_nuno_domingues.html',
    affiliation: 'Instituto Superior de Engenharia de Lisboa (ISEL)'
  }
];
...
async function runSetup() {
  for (const m of members) {
    ...
    const htmlContent = generateHtml(m.name, m.email);
    const filePath = path.join(outputDir, m.filename);
    fs.writeFileSync(filePath, htmlContent, 'utf8');
    console.log(`✅ Generated template: ${filePath}`);

    // Register primary email in Supabase Auth
    const { data: d1, error: e1 } = await supabase.auth.signUp({
      email: m.email,
      password: tempPassword,
      options: { data: { full_name: m.name } }
    });
    if (e1 && !e1.message.includes('already registered')) {
      console.log(`Notice for ${m.email}:`, e1.message);
    } else {
      console.log(`✅ Registered Auth account for ${m.email}`);
    }

    // Register secondary email as well
    if (m.secondary_email) {
      const { data: d2, error: e2 } = await supabase.auth.signUp({
        email: m.secondary_email,
        password: tempPassword,
        options: { data: { full_name: m.name } }
      });
      if (e2 && !e2.message.includes('already registered')) {
        console.log(`Notice for ${m.secondary_email}:`, e2.message);
      } else {
        console.log(`✅ Registered Auth account for ${m.secondary_email}`);
      }
    }
  }
}
